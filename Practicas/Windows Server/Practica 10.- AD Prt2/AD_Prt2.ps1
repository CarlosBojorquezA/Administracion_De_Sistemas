...
function Configurar-DominioAD(){
    if((Get-WmiObject Win32_ComputerSystem).Domain -eq "barcaeliminado.com"){
        echo "El dominio ya se encuentra configurado, omitiendo configuración..."
    }
    else{
        Import-Module ADDSDeployment
        Install-ADDSForest -DomainName "barcaeliminado.com" -DomainNetbiosName "BARCAELIMINADO" -InstallDNS
    }
}
...
function Crear-Usuario() {
    $dominio = "barcaeliminado.com"

    try {
        $nombreUsuario = Read-Host "Ingresa el nombre de usuario"
        $contrasena = Read-Host "Ingresa la contraseña"
        $grupo = Read-Host "Ingresa el grupo al que será parte el usuario (grupo1/grupo2)"

        if (($grupo -ne "grupo1") -and ($grupo -ne "grupo2")) {
            Write-Host "Ingresa un grupo válido (grupo1/grupo2)" -ForegroundColor Red
            return
        }

        if (-not (Es-ContrasenaValida -contrasena $contrasena)) {
            Write-Host "La contraseña no cumple con los requisitos de seguridad." -ForegroundColor Red
            return
        }

        # Verificar si el usuario ya existe
        if (Get-ADUser -Filter {SamAccountName -eq $nombreUsuario} -ErrorAction SilentlyContinue) {
            Write-Host "El usuario '$nombreUsuario' ya existe en el dominio. Omitiendo creación..." -ForegroundColor Yellow
            return
        }

        # Verificar si la OU existe
        $ouPath = "OU=$grupo,DC=$($dominio.Split('.')[0]),DC=$($dominio.Split('.')[1])"
        if (-not (Get-ADOrganizationalUnit -Filter "Name -eq '$grupo'" -ErrorAction SilentlyContinue)) {
            Write-Host "La Unidad Organizativa '$grupo' no existe. Crea la OU primero." -ForegroundColor Red
            return
        }
        ...
        New-ADUser `
            ...
            -Surname $nombreUsuario `
            -SamAccountName $nombreUsuario `
            -UserPrincipalName "$nombreUsuario@$dominio" `
            -Path $ouPath `
            -AccountPassword (ConvertTo-SecureString $contrasena -AsPlainText -Force) `
        ...
        # Configurar horario de acceso
        Configurar-Horarios -nombreUsuario $nombreUsuario -grupo $grupo

        Write-Host "Usuario '$nombreUsuario' creado y configurado correctamente." -ForegroundColor Green
    }
    catch {
...
function Configurar-PermisosAplicaciones(){ 
    try {
        ...
        if(Get-GPO -Name "Bloquear solo notepad" -ErrorAction SilentlyContinue){
            ...
            New-GPLink -Name "Bloquear solo notepad" -Target "OU=grupo2,DC=barcaeliminado,DC=com"
        ...
        if(Get-GPO -Name "Permitir solo notepad" -ErrorAction SilentlyContinue){
            ...
            New-GPLink -Name "Permitir solo notepad" -Target "OU=grupo1,DC=barcaeliminado,DC=com"
...
function Configurar-Auditoria(){
    try {
        ...
        if (-not (Get-GPO -Name $nombreGpo -ErrorAction SilentlyContinue)) {
            ...
            New-GPLink -Name $nombreGpo -Target "DC=barcaeliminado,DC=com"
...
function Configurar-ContrasenasSeguras(){
    try {
        Set-ADDefaultDomainPasswordPolicy -Identity "barcaeliminado.com" `
        -MinPasswordLength 8 `
...
function Configurar-AlmacenamientoArchivos(){
    ...
    try {
        if (Get-GPO -Name "Cuota5MbGrupoUno" -ErrorAction SilentlyContinue) {
            ...
            New-GPO -Name $gpo1
            New-GPLink -Name $gpo1 -Target "OU=grupo1,DC=barcaeliminado,DC=com" -Enforced "Yes"
        ...
        if (Get-GPO -Name "Cuota10MbGrupo2" -ErrorAction SilentlyContinue) {
            ...
            New-GPO -Name 
            New-GPLink -Name $gpo2 -Target "OU=grupo2,DC=barcaeliminado,DC=com" -Enforced "Yes"
